...
> **필수 단계입니다.** 플러그인 설치만으로는 인증이 자동으로 진행되지 않습니다. 아래 절차를 반드시 수행해야 합니다.
...
> 인증 토큰은 자동으로 갱신됩니다. 장기간 미사용 후 토큰이 만료되면 동일한 `/mcp` 절차로 다시 인증하면 됩니다.
...
### Claude Code 플러그인과 Claude.ai 커넥터를 동시에 사용하지 마세요

> ⚠️ Claude Code 플러그인과 Claude.ai 웹 커넥터를 함께 사용하면 **로그인 계정이 플러그인 쪽으로 강제 고정**됩니다. 의도한 계정과 다른 계정으로 작업이 진행되어 잘못된 프로젝트가 조회·배포되는 사고가 발생할 수 있습니다.

**증상**

- Claude.ai 커넥터에서 A 계정으로 로그인했는데, 실제 호출 결과는 B 계정(=플러그인에 인증된 계정)의 데이터가 나옴
- 커넥터로 재로그인해도 계정이 바뀌지 않음

**해결: 커넥터를 사용하려면 플러그인을 완전히 제거해야 합니다**

1. Claude Code 터미널에서 플러그인 제거
   ```
...
2. Claude Code 완전 종료
   ...
   exit
   ```
3. Claude.ai 웹에서 새 대화를 시작한 뒤 첫 프롬프트에 다음과 같이 명시
   ```
   Cafe24 AI SPACE 플러그인은 사용하지 않습니다. Claude 커넥터로만 진행해주세요.
   ```

> 반대로 Claude Code 플러그인을 다시 사용하고 싶을 때는 §2 설치 절차를 다시 따라가면 됩니다.
...
**동시 사용은 권장하지 않습니다.** 둘을 함께 켜두면 로그인 계정이 플러그인 쪽으로 강제 고정되어 의도와 다른 계정으로 작업이 진행될 수 있습니다. 커넥터를 사용하려면 플러그인을 완전히 제거하고(`/plugin uninstall cafe24-aispace` 및 marketplace remove) Claude Code를 종료한 뒤, Claude.ai 새 대화에서 "Cafe24 AI SPACE 플러그인은 사용하지 않습니다. Claude 커넥터로만 진행해주세요"라고 명시해주세요. 자세한 절차는 §6의 "Claude Code 플러그인과 Claude.ai 커넥터를 동시에 사용하지 마세요" 항목 참조.
